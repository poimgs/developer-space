...
func (r *SessionRepository) Create(ctx context.Context, req model.CreateSessionRequest, createdBy uuid.UUID) (*model.SpaceSession, error) {
	...
	err := r.pool.QueryRow(ctx,
		`INSERT INTO space_sessions (title, description, date, start_time, end_time, capacity, location, status, series_id, created_by)
		...
		req.Title, req.Description, req.Date, req.StartTime, req.EndTime, req.Capacity, req.Location, req.SeriesID, createdBy,
...
func (r *SessionRepository) List(ctx context.Context, from, to, status string, memberID *uuid.UUID) ([]model.SpaceSession, error) {
	...
	for rows.Next() {
		var s model.SpaceSession
		if err := rows.Scan(&s.ID, &s.Title, &s.Description, &s.Date, &s.StartTime, &s.EndTime, &s.Capacity, &s.Status, &s.ImageURL, &s.Location, &s.SeriesID, &s.CreatedBy, &s.CreatedAt, &s.UpdatedAt, &s.RSVPCount, &s.UserRSVPed); err != nil {
...
func (r *SessionRepository) GetByID(ctx context.Context, id uuid.UUID, memberID *uuid.UUID) (*model.SpaceSession, error) {
	...
	).Scan(&s.ID, &s.Title, &s.Description, &s.Date, &s.StartTime, &s.EndTime, &s.Capacity, &s.Status, &s.ImageURL, &s.Location, &s.SeriesID, &s.CreatedBy, &s.CreatedAt, &s.UpdatedAt, &s.RSVPCount, &s.UserRSVPed)
	if err != nil {
...
func (r *SessionRepository) Update(ctx context.Context, id uuid.UUID, req model.UpdateSessionRequest, newStatus *string) (*model.SpaceSession, error) {
	...
	err := r.pool.QueryRow(ctx, query, args...).Scan(
		&s.ID, &s.Title, &s.Description, &s.Date, &s.StartTime, &s.EndTime, &s.Capacity, &s.Status, &s.ImageURL, &s.Location, &s.SeriesID, &s.CreatedBy, &s.CreatedAt, &s.UpdatedAt,
	)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, nil
...
func (r *SessionRepository) Cancel(ctx context.Context, id uuid.UUID) (*model.SpaceSession, error) {
	...
	err := r.pool.QueryRow(ctx,
		`UPDATE space_sessions SET status = 'canceled', updated_at = now()
		 WHERE id = $1
		 RETURNING id, title, description, date::text, to_char(start_time, 'HH24:MI'), to_char(end_time, 'HH24:MI'), capacity, status, image_url, location, series_id, created_by, created_at, updated_at`, id,
	).Scan(&s.ID, &s.Title, &s.Description, &s.Date, &s.StartTime, &s.EndTime, &s.Capacity, &s.Status, &s.ImageURL, &s.Location, &s.SeriesID, &s.CreatedBy, &s.CreatedAt, &s.UpdatedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, nil
